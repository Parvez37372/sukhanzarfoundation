...
$sql = "SELECT p.id, p.name, p.image, p.dob, p.location, 
               COUNT(po.id) AS sher_count
        FROM poets_categories p
        LEFT JOIN poetry po ON p.id = po.poet_id
        WHERE 1";
...
// Alphabet filter
if (!empty($letter)) {
    $sql .= " AND p.name LIKE '" . $conn->real_escape_string($letter) . "%'";
}
...
<div class="container mt-4">
    ...
    <div class="alphabet-filter">
        ...
        <?php endforeach; ?>
        <a href="poets_categories_list.php" style="background:#555; color:#fff;">All</a>
    </div>

    <!-- 📖 Poet List -->
    <?php if ($result && $result->num_rows > 0) { 
        while($row = $result->fetch_assoc()) { ?>
            <a href="sher_list.php?id=<?= $row['id']; ?>" style="text-decoration:none; color:inherit;">
                <div class="post-card">
                    ...
                    <div class="post-info">
                        ...
                        <p>🎂 <?= $row['dob'] ?: 'N/A'; ?></p>
                        <p>✍️ Sher Count: <b><?= $row['sher_count']; ?></b></p>
                    </div>
                </div>
            </a>
        <?php } 
